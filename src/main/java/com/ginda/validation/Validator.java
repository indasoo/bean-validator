...
public class Validator {
     ...
     *
     * @param Obj 数据
     * @throws ValidationException 数据检验异常
     */
            ...
            if (field.isAnnotationPresent(ValidateData.class)) {
                ...
                if (value == null) {
                    ...
                    if (validateData.required()) {
                        throw new ValidationException(validateData.name() + "不能为空");
                    }
                } else {
                    if (!isWrapClassData(value)) {
                        ...
                        if (StringUtils.isNotEmpty(validateData.constantValue())
                            ...
                            throw new ValidationException(validateData.name() + "与约定值不一致");
                        }

                        // 长度检查
                        if (validateData.minLength() != 0 && strValue.length() < validateData.minLength()) {
                            throw new ValidationException(validateData.name() + "长度不能小于" + validateData.minLength());
                        }

                        if (validateData.maxLength() != Integer.MAX_VALUE && strValue.length() > validateData.maxLength()) {
                            throw new ValidationException(validateData.name() + "长度不能超过" + validateData.maxLength());
                        }

                        // 数据类型验证
                        if (validateData.regexType() != RegexType.NONE) {
                            if (!Pattern.matches(validateData.regexType().regex(), strValue)) {
                                throw new ValidationException(validateData.name() + "格式不正确");
                            }
                        }
                        ...
                        if (validateData.valueRangeEnumClazz() != ValidateData.EnumClazz.class) {
                            ...
                            if (!isExist) {
                                throw new ValidationException(validateData.name() + "不合法");
                            }
                        }
                    }
                }
